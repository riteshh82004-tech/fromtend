...
import { X, Heart, Activity, Droplet, Calendar, Play, CheckCircle, Usb, Bluetooth, AlertCircle } from 'lucide-react';
...
export const TestFlow: React.FC<TestFlowProps> = ({ onClose }) => {
  ...
  const [step, setStep] = useState<'selection' | 'instructions' | 'connection' | 'recording' | 'results'>('selection');
  const [selectedTest, setSelectedTest] = useState<'ecg' | 'spo2' | 'glucose' | 'period' | null>(null);
  ...
  useEffect(() => {
    if (selectedTest === 'ecg' && ecgRecording && ecgData.length > 0) {
      // Add new samples to ECG session
      const newSamples = ecgData.slice(-10); // Get last 10 samples
  ...
  useEffect(() => {
    if (selectedTest === 'ecg' && recording && deviceStatus === 'connected') {
      const duration = 30; // 30 seconds for ECG
  ...
  const handleECGConnection = async (mode: 'usb' | 'bluetooth') => {
    ...
    try {
      ...
      if (deviceStatus === 'connected') {
        if (user) {
          startSession(user.id, 250); // 250 Hz sampling rate
        }
        startStreaming();
        setStep('recording');
        setRecording(true);
  ...
  const handleECGStop = () => {
    ...
    // Generate result from ECG session
    if (currentSession && currentSession.samples.length > 0) {
      const heartRate = currentSession.avgHeartRate || calculateHeartRateFromSamples(currentSession.samples);
      const status = heartRate >= 60 && heartRate <= 100 ? 'normal' : heartRate > 100 ? 'warning' : 'danger';
      setResult({ value: heartRate, status });
      
      // Add reading to patient store
      if (user) {
        addReading({
          ...
          notes: `ECG session - ${currentSession.samples.length} samples, Quality: ${currentSession.signalQuality || 'good'}`
        });
      }
    }
  ...
  const calculateHeartRateFromSamples = (samples: number[]): number => {
    ...
    for (let i = 1; i < samples.length - 1; i++) {
      if (samples[i] > threshold && samples[i] > samples[i - 1] && samples[i] > samples[i + 1] && i - lastPeakIndex > 20) {
        peaks.push(i);
        lastPeakIndex = i;
      }
    }
    
    if (peaks.length < 2) return 72; // Default
    ...
    return Math.round((60 * 250) / avgInterval); // 250 Hz sampling rate
  };
  ...
  const generateResult = () => {
    ...
    switch (selectedTest) {
      case 'ecg':
        ...
        status = value >= 60 && value <= 100 ? 'normal' : value > 100 ? 'warning' : 'danger';
        break;
      case 'spo2':
  ...
  const renderTestSelection = () => (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Select Test Type</h2>
        <p className="text-gray-600">Choose which health metric you'd like to measure</p>
      </div>
        ...
        {tests.map((test) => {
          ...
          return (
            <motion.button
              ...
              className="p-6 rounded-3xl bg-white border border-gray-200 hover:border-gray-300 transition-all duration-200"
            >
              <div className={`w-16 h-16 bg-gradient-to-r ${test.color} rounded-2xl flex items-center justify-center mx-auto mb-4`}>
                <Icon className="w-8 h-8 text-white" />
              </div>
              <h3 className="font-semibold text-gray-800 mb-1">{test.name}</h3>
              <p className="text-sm text-gray-600 mb-2">{test.description}</p>
              <div className="text-xs text-blue-600 font-medium">{test.duration}</div>
            </motion.button>
          );
        })}
      </div>
  ...
  const renderInstructions = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <div className={`w-20 h-20 bg-gradient-to-r ${test.color} rounded-2xl flex items-center justify-center mx-auto mb-4`}>
            <Icon className="w-10 h-10 text-white" />
          </div>
          <h2 className="text-2xl font-bold text-gray-800 mb-2">{test.name} Instructions</h2>
          <p className="text-gray-600">Follow these steps for accurate results</p>
        </div>
  ...
  const renderConnection = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-800 mb-2">Connect ECG Device</h2>
        ...
        <div className="grid grid-cols-1 gap-4">
          <motion.button
            ...
            className="p-6 rounded-2xl bg-white border-2 border-gray-200 hover:border-blue-500 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1 text-left">
                <h3 className="font-semibold text-gray-800">USB Connection</h3>
                <p className="text-sm text-gray-600">Connect via USB cable (Recommended)</p>
              </div>
          ...
          >
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1 text-left">
                <h3 className="font-semibold text-gray-800">Bluetooth Connection</h3>
                <p className="text-sm text-gray-600">Connect wirelessly via Bluetooth</p>
              </div>
  ...
  const renderRecording = () => {
    ...
    return (
      <div className="space-y-8 text-center">
        <div>
          <div className={`w-24 h-24 bg-gradient-to-r ${test.color} rounded-3xl flex items-center justify-center mx-auto mb-4`}>
            <Icon className="w-12 h-12 text-white" />
          </div>
          <h2 className="text-2xl font-bold text-gray-800 mb-2">Recording {test.name}</h2>
          <p className="text-gray-600">Please remain still and follow the instructions</p>
          {selectedTest === 'ecg' && deviceStatus === 'connected' && (
            <p className="text-sm text-green-600 mt-2">Device connected via {deviceMode?.toUpperCase()}</p>
          )}
        </div>
        ...
        <div className="flex justify-center">
          {selectedTest === 'ecg' && (
            <div className="w-full">
              <Waveform animated color="#ef4444" height={200} width={600} showGrid={true} />
              {currentSession && (
                <div className="mt-4 text-sm text-gray-600">
                  Samples: {currentSession.samples.length} | 
                  {currentSession.avgHeartRate && ` Heart Rate: ${currentSession.avgHeartRate} bpm`}
                </div>
              )}
            </div>
          )}
          {selectedTest === 'spo2' && <Pulse color="#3b82f6" value={Math.round(95 + progress / 20)} />}
        ...
        <div className="space-y-3">
          ...
          <p className="text-lg font-semibold text-gray-700">{Math.round(progress)}% Complete</p>
        </div>

        {selectedTest === 'ecg' && recording && (
          <Button onClick={handleECGStop} size="lg" className="w-full" variant="outline">
            Stop Recording
          </Button>
        )}

        {!recording && progress === 0 && selectedTest !== 'ecg' && (
          <Button onClick={() => setRecording(true)} size="lg" className="w-full">
            Start Recording
          </Button>
        )}
      </div>
    ...
    return (
      <div className="space-y-8 text-center">
        <div>
          ...
          <p className="text-gray-600">Your {test.name.toLowerCase()} results are ready</p>
        </div>

        <Card className="bg-gradient-to-br from-gray-50 to-gray-100 border-0">
          <div className="text-center space-y-4">
            <div className={`w-20 h-20 bg-gradient-to-r ${test.color} rounded-2xl flex items-center justify-center mx-auto`}>
              <Icon className="w-10 h-10 text-white" />
            </div>
            ...
            <div>
              ...
              >
                {result.value} <span className="text-lg font-normal text-gray-600">{getUnit(selectedTest!)}</span>
              </motion.div>
              <div className={`text-lg font-semibold ${statusColors[result.status]} mb-4`}>
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-xl font-bold text-gray-800">Health Test</h1>
          <motion.button
