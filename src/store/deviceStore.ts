...
interface DeviceState {
  ...
  port: SerialPort | BluetoothRemoteGATTCharacteristic | null;
...
export const useDeviceStore = create<DeviceState>((set, get) => {
  ...
  const readUSBData = async (port: SerialPort, reader: ReadableStreamDefaultReader<Uint8Array>) => {
    try {
  ...
  return {
    ...
    disconnect: () => {
      ...
      if (state.mode === 'usb' && state.port) {
        const port = state.port as SerialPort;
        if (state.reader) {
      ...
      if (state.mode === 'bluetooth' && state.port) {
        const characteristic = state.port as BluetoothRemoteGATTCharacteristic;
        characteristic.stopNotifications().catch(console.error);
        characteristic.service?.device?.gatt?.disconnect().catch(console.error);
      }
    ...
    startStreaming: () => {
      ...
      if (state.mode === 'usb' && state.port && state.reader) {
        readUSBData(state.port as SerialPort, state.reader);
      }
